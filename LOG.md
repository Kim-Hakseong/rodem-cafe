...
## Phase 8.18 — 예약 주문 사전 표시 (봉사자·주문자)
- **상태:** ✅ 완료 (운영 배포 · 실사용 확인, PR #1)
- **완료일:** 2026-09-19
- **배경:** 예약 주문이 예약 시간이 되어야 대기열에 나타나서, 봉사자도 예약자 본인도 예약 여부를 확인할 수 없었음.
- **산출물:**
  - **OrderQueue 예약 섹션**: 예약 시간 전 대기 주문을 숨기던 필터 제거 → 대기열 상단 보라색 `🕐 예약 주문` 칸에 표시
    - 예약 시간 이른 순 정렬, 카드에 `예약시각 · N분 후` 표시, 결제수단 뱃지 포함
    - 예약 시간이 되면 자동으로 일반 대기 목록으로 이동 (3초 폴링 시 판정)
    - 헤더에 `예약 N건`, 접힌 탭에 보라색 예약 카운트 뱃지
    - 봉사자 모드: `미리 완료` / `예약 취소`(기존 반려 API) 버튼
    - 주문자 모드: 버튼 없이 조회만 → 예약자 본인이 확인 가능
  - **음성 알림(봉사자)**: 예약 접수 시 "OO님 HH:MM 예약 주문이 접수되었습니다", 예약 시간 도래 시 "OO님 예약 시간입니다. … 준비해주세요"
    - 고객 모드 완료 알림은 예약 주문을 미리 완료한 경우도 포함
  - **예약 완료 안내**: 토스트에 예약 시각 표시 + 대기열 자동 열림, 음성 "HH:MM 예약이 접수되었습니다"
  - **개인페이지(`/my/[token]`)**: 상단에 `🕐 예약된 주문` 카드 표시
- **변경 파일:** `OrderQueue.tsx`, `OrderConfirm.tsx`, `pos/page.tsx`, `my/[token]/page.tsx`
- **검증:** `tsc --noEmit` 통과, `next build` 통과 (DB 스키마 변경 없음)
- **이슈:** 로컬 `node_modules` 링크 손상 → 삭제 후 `pnpm install --frozen-lockfile`로 재설치
- **다음:** Phase 8.19

## Phase 8.19 — 예약 시각 표시 확대
- **상태:** ✅ 완료
- **완료일:** 2026-09-19
- **배경:** 운영 피드백 — 예약 시각 글씨가 작아 봉사자가 못 보고 바로 제조할 우려.
- **산출물:**
  - 예약 카드 상단에 보라색 배경 띠로 `🕐 HH:MM 예약`을 `2.75rem` 굵게 표시 (메뉴명보다 크게) + `N분 후`
  - 카드 우상단에 `⏳ 아직 만들지 마세요` 안내
  - 예약 시간이 되어 대기 목록으로 넘어간 주문의 예약 뱃지도 `text-sm` → `text-2xl` 보라색 알약형으로 확대
- **변경 파일:** `OrderQueue.tsx`
- **검증:** `tsc --noEmit` 통과, `next build` 통과
- **이슈:** 없음
- **다음:** 베타 운영 계속
