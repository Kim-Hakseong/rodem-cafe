  ...
  return (
    <div className="p-4">
      <div className="text-center mb-6">
        <div className="text-base text-rodem-text-sub mb-1">{member.name}님의 결제</div>
        <div className="text-[26px] font-bold text-rodem-text">{formatPrice(cartTotal)}</div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {PAYMENT_METHODS.map((pm) => (
          ...
          >
            <div className="text-[32px] mb-2">{pm.icon}</div>
            ...
            {pm.id === 'prepaid' && (
              <div className="text-[15px] mt-1 text-rodem-text-sub">
      ...
      <button
        onClick={onBack}
        className="w-full py-3 rounded-rodem-sm border border-rodem-border-light bg-white text-rodem-text-sub font-semibold cursor-pointer text-base"
      >
        ← 메뉴 선택으로 돌아가기
      </button>
      ...
      {showTransferInfo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowTransferInfo(false)}>
          ...
          <div className="relative bg-gradient-to-br from-[#fefcf9] to-[#f8f4ec] rounded-rodem p-6 max-w-sm w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-xl font-bold text-rodem-text mb-4">🏦 계좌이체 안내</h3>
            ...
            {mode === 'staff' ? (
              <button
                ...
                className="w-full py-3 rounded-rodem-sm bg-gradient-to-br from-[#4a7fd4] to-[#3a6fc4] text-white font-bold cursor-pointer text-base"
              >
                이체 확인 완료
              </button>
            ) : (
              <>
                <p className="text-center text-base text-rodem-text-sub mb-3">
                  위 계좌로 이체해 주세요.<br />봉사자가 입금을 확인합니다.
                </p>
                <button
                  onClick={handleTransferConfirm}
                  className="w-full py-3 rounded-rodem-sm bg-gradient-to-br from-[#4a7fd4] to-[#3a6fc4] text-white font-bold cursor-pointer text-base"
      ...
      {showPrepaidShortage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowPrepaidShortage(false)}>
          <div className="absolute inset-0 bg-[rgba(74,69,65,0.4)] backdrop-blur-[10px]" />
          <div className="relative bg-gradient-to-br from-[#fefcf9] to-[#f8f4ec] rounded-rodem p-6 max-w-sm w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-xl font-bold text-rodem-text mb-2">💰 선불 잔액 부족</h3>
            <div className="text-base text-rodem-text-sub mb-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                ...
                className="w-full py-3 rounded-rodem-sm bg-rodem-blue-light border border-rodem-blue text-rodem-blue font-bold cursor-pointer text-base"
              >
                🏦 부족분 계좌이체 ({formatPrice(shortage)})
              </button>
              <button
                onClick={() => setShowPrepaidShortage(false)}
                className="w-full py-2 text-rodem-text-sub text-base cursor-pointer bg-transparent border-none"
              >
                취소
              </button>
            </div>
