  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-rodem-text-sub">
          {date ? '조회 버튼을 눌러주세요' : '주문이 없습니다'}
        </div>
      ) : (
                    ...
                    <div className="flex gap-2 pt-1">
                      {order.status === 'pending' && (
                        <>
                          <button
                            ...
                            className="flex-1 py-2 rounded-lg bg-rodem-green-light border border-rodem-green text-rodem-green text-sm font-bold cursor-pointer"
                          >
                            완료 처리
                          </button>
                          <button
                            onClick={() => handleCancel(order.id)}
                            className="flex-1 py-2 rounded-lg bg-red-50 border border-rodem-red text-rodem-red text-sm font-bold cursor-pointer"
                          >
                            반려
                          </button>
                        </>
                      ...
                      {order.status === 'cancelled' && (
                        <button
                          ...
                          className="flex-1 py-2 rounded-lg bg-rodem-gold-light border border-rodem-gold text-rodem-gold text-sm font-bold cursor-pointer"
                        >
                          대기로 복원
                        </button>
                      )}
                      <button
                        ...
                        className="py-2 px-4 rounded-lg bg-red-50 border border-rodem-red text-rodem-red text-sm font-bold cursor-pointer"
                      >
                        삭제
                      </button>
                    </div>
