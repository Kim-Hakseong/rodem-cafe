...
export default function MyPage() {
  ...
  if (notFound) return <div className="min-h-screen flex items-center justify-center bg-rodem-bg font-sans"><div className="text-center"><div className="text-4xl mb-4">🔍</div><div className="text-rodem-text font-bold">찾을 수 없습니다</div><div className="text-sm text-rodem-text-sub mt-2">유효하지 않은 QR코드입니다</div></div></div>
  ...
  if (!authenticated && hasPin) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-gradient-to-br from-[#efebe4] via-[#e5e0d8] to-[#dedad2] font-sans">
        ...
        <h2 className="text-xl font-bold mb-2 text-rodem-text">{member?.name}님</h2>
        <p className="text-sm text-rodem-text-sub mb-8">개인 PIN 4자리를 입력하세요</p>
  ...
  return (
    <div className="min-h-screen bg-rodem-bg font-sans">
      <div className="bg-gradient-to-br from-[#4a4541] to-[#3a3632] text-white p-5 text-center">
        ...
        <p className="text-xs opacity-70 mt-1">로뎀나무 카페</p>
      </div>

      <div className="p-4">
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="p-4 rounded-rodem-sm bg-rodem-orange-light border border-rodem-orange/20">
            <div className="text-xs text-rodem-orange mb-1">📋 외상</div>
            <div className="text-xl font-bold text-rodem-orange">{formatPrice(member?.credit_balance ?? 0)}</div>
          </div>
          <div className="p-4 rounded-rodem-sm bg-rodem-purple-light border border-rodem-purple/20">
            <div className="text-xs text-rodem-purple mb-1">💰 선불</div>
            <div className="text-xl font-bold text-rodem-purple">{formatPrice(member?.prepaid_balance ?? 0)}</div>
          </div>
        </div>

        <h3 className="font-bold text-base text-rodem-text mb-3">최근 주문</h3>
        <div className="space-y-2">
          ...
          {orders.length === 0 && <div className="text-center py-8 text-rodem-text-sub text-sm">주문 이력이 없습니다</div>}
        </div>
      </div>
    </div>
  )
