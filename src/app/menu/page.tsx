  ...
  if (!authenticated) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-gradient-to-br from-[#efebe4] via-[#e5e0d8] to-[#dedad2] font-sans relative overflow-hidden">
        ...
        <button onClick={() => router.push('/')} className="absolute top-4 left-4 bg-gradient-to-br from-[#f0ece4] to-[#e8e3da] border-none text-sm text-rodem-text-sub cursor-pointer py-2 px-3.5 rounded-[10px] z-10">
          &larr; 뒤로
        </button>
        <div className="text-[40px] mb-4 relative z-10">☕</div>
        <h2 className="text-[22px] font-bold mb-2 text-rodem-text relative z-10">메뉴 관리</h2>
        <p className="text-sm text-rodem-text-sub mb-8 relative z-10">관리자 PIN을 입력하세요</p>
        {pinError && <p className="text-rodem-red text-sm font-semibold mb-3 relative z-10">PIN이 틀렸습니다</p>}
  ...
  return (
    <div className="min-h-screen bg-rodem-bg font-sans">
      <Header
        ...
        onBack={() => router.push('/')}
        right={
          <button onClick={openAdd} className="bg-rodem-gold border-none text-white py-1.5 px-4 rounded-[10px] text-sm font-bold cursor-pointer shadow-[0_2px_8px_rgba(201,162,39,0.3)]">
            + 추가
          </button>
        }
      />
      ...
      <div className="flex gap-2 px-4 py-3 overflow-x-auto">
        {MENU_CATS.map((cat) => (
          <button key={cat} onClick={() => setActiveCategory(cat)}
            className={cn('flex-shrink-0 py-2 px-4 rounded-[12px] text-sm font-bold cursor-pointer border transition-all',
            ...
            {cat}
            <span className={cn('ml-1.5 text-[11px]', activeCategory === cat ? 'opacity-80' : 'text-rodem-text-sub')}>
      ...
      <div className="px-4 pb-6">
        {loading ? (
          ...
          <div className="text-center py-12 text-rodem-text-sub text-sm">
            <div className="text-3xl mb-3">🍽</div>
            <p>{activeCategory} 카테고리에 메뉴가 없습니다</p>
            <button onClick={openAdd} className="mt-4 py-2 px-5 rounded-[12px] bg-rodem-gold text-white font-bold text-sm cursor-pointer border-none">첫 메뉴 추가하기</button>
          </div>
        ) : (
              ...
              <div key={item.id} className={cn('flex items-center gap-3 p-3.5 rounded-rodem-sm border transition-all',
                ...
                <div className="flex flex-col gap-0.5">
                  <button onClick={() => changeOrder(item, 'up')} disabled={idx === 0} aria-label="위로 이동"
                    className="w-7 h-6 rounded-[6px] bg-rodem-border-light text-rodem-text-sub text-[11px] flex items-center justify-center cursor-pointer border-none disabled:opacity-30">↑</button>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-sm text-rodem-text truncate">{item.name}</span>
                    {item.temp_type && (
                      <span className={cn('text-[10px] font-bold px-1.5 py-0.5 rounded-[5px]', TEMP_BADGE[item.temp_type])}>{item.temp_type}</span>
                    )}
                  </div>
                ...
                <button onClick={() => openEdit(item)} aria-label="수정"
                  className="w-8 h-8 rounded-[8px] bg-rodem-border-light text-rodem-text-sub text-xs flex items-center justify-center cursor-pointer border-none flex-shrink-0">
                  ✏️
                </button>
              </div>
            ))}
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editTarget ? '메뉴 수정' : '메뉴 추가'} maxWidth="max-w-md">
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-rodem-text-sub mb-1.5">메뉴 이름 *</label>
            <input type="text" value={form.name} onChange={(e) => setField('name', e.target.value)} placeholder="예) 아메리카노"
              className="w-full p-3 rounded-rodem-sm border-2 border-rodem-border-light bg-rodem-bg text-rodem-text text-sm font-sans focus:outline-none focus:border-rodem-gold" />
          </div>
          <div>
            <label className="block text-xs font-bold text-rodem-text-sub mb-1.5">가격 (원) *</label>
            <input type="number" value={form.price} onChange={(e) => setField('price', e.target.value)} placeholder="예) 2000"
              className="w-full p-3 rounded-rodem-sm border-2 border-rodem-border-light bg-rodem-bg text-rodem-text text-sm font-sans focus:outline-none focus:border-rodem-gold" />
          </div>
          <div>
            <label className="block text-xs font-bold text-rodem-text-sub mb-1.5">카테고리 *</label>
            <div className="flex flex-wrap gap-1.5">
              {MENU_CATS.map((cat) => (
                <button key={cat} onClick={() => setField('category', cat)}
                  className={cn('py-1.5 px-3 rounded-[10px] text-xs font-bold cursor-pointer border', form.category === cat ? 'bg-rodem-gold text-white border-rodem-gold' : 'bg-rodem-card text-rodem-text-sub border-rodem-border-light')}>
                  {cat}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="block text-xs font-bold text-rodem-text-sub mb-1.5">온도 (선택)</label>
            <div className="flex gap-1.5">
              {(['HOT', 'ICE', null] as const).map((t) => (
                <button key={String(t)} onClick={() => setField('temp_type', t)}
                  className={cn('py-1.5 px-3.5 rounded-[10px] text-xs font-bold cursor-pointer border',
                    form.temp_type === t
          ...
          <div>
            <label className="block text-xs font-bold text-rodem-text-sub mb-1.5">정렬 순서</label>
            <input type="number" value={form.sort_order} onChange={(e) => setField('sort_order', e.target.value)}
              className="w-full p-3 rounded-rodem-sm border-2 border-rodem-border-light bg-rodem-bg text-rodem-text text-sm font-sans focus:outline-none focus:border-rodem-gold" />
          </div>
          <div className="flex gap-2 pt-2">
            <button onClick={closeModal} className="flex-1 py-3 rounded-rodem-sm border border-rodem-border-light bg-rodem-card text-rodem-text-sub font-bold text-sm cursor-pointer">취소</button>
            <button onClick={handleSubmit} disabled={!form.name.trim() || !form.price || submitting}
              className="flex-1 py-3 rounded-rodem-sm bg-gradient-to-br from-rodem-gold to-[#b8911f] text-white font-bold text-sm cursor-pointer border-none shadow-[0_4px_16px_rgba(201,162,39,0.25)] disabled:opacity-50">
